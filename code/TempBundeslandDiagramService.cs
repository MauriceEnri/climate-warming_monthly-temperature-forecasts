...
public class TempBundeslandDiagramService
{
    public void Create(Dictionary<string, List<(DateTime date, double daily, double cum)>> series)
    {
        ...
        {
            Title = $"Fortlaufendes Temperaturmittel – Bundesländer {startDate:dd.MM} – {endDate:dd.MM}",
        ...
        var valueAxis = new LinearAxis
        {
            ...
            MajorGridlineColor = OxyColor.FromRgb(120, 120, 120),
            MinorGridlineColor = OxyColor.FromRgb(150, 150, 150),
            Minimum = Math.Floor(minVal - 0.5),
            Maximum = Math.Ceiling(maxVal + 0.5),
            MinimumPadding = 0.05,
            MaximumPadding = 0.05
        };
        ...
        var palette = new[]
        {
            ...
            OxyColors.DarkRed, OxyColors.DarkBlue, OxyColors.DarkGreen,
            OxyColors.Goldenrod, OxyColors.Teal, OxyColors.SlateGray,
            OxyColors.Olive, OxyColors.IndianRed
        };
        ...
        foreach (var kv in series.OrderBy(k => k.Key))
        {
            ...
            {
                Title = $"{shortNames.GetValueOrDefault(bl, bl)} = {endCum:F2}",
        ...
        // WICHTIG: Platz rechts schaffen
        model.Padding = new OxyThickness(10, 10, 140, 10);

        model.Legends.Add(legend);


        Directory.CreateDirectory("output/diagramme/bundesland/temp");

        using var stream = File.Create("output/diagramme/bundesland/temp/bundeslaender_temp.png");
